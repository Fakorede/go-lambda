...
import (
	...
	"github.com/Fakorede/go-serverless/pkg/user"
...
func GetUser(req events.APIGatewayProxyRequest, tableName string, dynaClient dynamodbiface.DynamoDBAPI) (*events.APIGatewayProxyResponse, error) {
	...
	if len(email) > 0 {
		result, err := user.ReadUser(email, tableName, dynaClient)
		if err != nil {
			return apiResponse(http.StatusBadRequest, Error{
	...
	result, err := user.ReadUsers(tableName, dynaClient)
	if err != nil {
		return apiResponse(http.StatusBadRequest, Error{
			ErrorMsg: aws.String(err.Error()),
		})
...
func StoreUser(req events.APIGatewayProxyRequest, tableName string, dynaClient dynamodbiface.DynamoDBAPI) (*events.APIGatewayProxyResponse, error) {
